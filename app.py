...
async def events(request: Request):
    ...
    for psid, text in messenger.parse_events(payload):
        ...
        _BG.add(t)
        t.add_done_callback(_BG.discard)
    for comment_id, from_id, comment_text in messenger.parse_comment_events(payload):
        t = asyncio.create_task(messenger.handle_comment(comment_id, from_id, comment_text))
        _BG.add(t)
        t.add_done_callback(_BG.discard)
    return JSONResponse({"ok": True})
