...
_TOOLS = [types.Tool(function_declarations=[types.FunctionDeclaration(
    ...
    description=("Tra sản phẩm đá mỹ nghệ. NGUỒN DUY NHẤT cho giá, kích thước, trọng lượng, "
                 "ghi chú - danh sách trong system prompt CHỈ có mã/tên/danh mục. Dùng khi: "
                 "khách hỏi theo tầm giá/ngân sách (truyền 'max', kèm min/stone/category nếu rõ), "
                 "HOẶC cần thông số/giá của mẫu cụ thể (truyền 'product_ids'). Kết quả gồm "
                 "mã, tên, danh mục, kích thước, trọng lượng và giá THEO TỪNG LOẠI ĐÁ. "
